...
const systemPromptBase = `You are the AI assistant built into eTerm, an SSH client and terminal multiplexer. You help the user operate terminals and remote daemons.
...
- notify: send a desktop notification (OSC 9) to the user's machine when they asked to be notified about an event. The popup can be missed or unsupported, so always also report the same information in your reply.
...
func agentInstruction(daemons bool) string {
	s := systemPromptBase
	if daemons {
		s += daemonToolsPrompt
	}
