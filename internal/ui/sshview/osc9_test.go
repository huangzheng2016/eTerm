...
func batchForChunk(t *testing.T, m *Model, data string) tea.BatchMsg {
	t.Helper()
	m.ch <- []byte("next")
...
func expectOnlyWaitChunk(t *testing.T, m *Model, data string) {
	t.Helper()
	m.ch <- []byte("next")
	_, cmd := m.Update(ChunkMsg{StreamID: m.StreamID(), Data: []byte(data)})
	if cmd == nil {
	...
	if _, ok := cmd().(ChunkMsg); !ok {
		t.Fatal("unexpected notification command")
	}
}
...
func TestOSC9InvalidPayloadDoesNotNotify(t *testing.T) {
	m := newTestModel(t, nil)
	expectOnlyWaitChunk(t, m, "\x1b]9\a\x1b]9;\a")
}
